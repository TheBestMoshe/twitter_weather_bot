...
while not network_connection:
    try:
        ...
        if connection_attempts == 1:
            ...
            logger.info(f'It took {connection_attempts/2} seconds to connect'
                        ' to network.')
    except Exception:
...
# Schedule jobs
scheduler.add_job(morning_report, trigger='cron',
                  day_of_week='mon-fri, sun', hour=7)
scheduler.add_job(evening_report, trigger='cron',
                  day_of_week='sun, wed', hour=20)
scheduler.add_job(evening_report, trigger='cron',
                  day_of_week='fri', hour=14)


# Send me a DM letting me know that the bot instance has started
logger.debug('Sending startup DM')
parts.dm('moshe_grunwald', f'{version} version of LkwdWeatherBot has been '
         'started')
...
# Once we started the scheduler, we will begin checking if the script has been
#   changed
...
while True:
    for f, mtime in watched_files_mtimes:
        if os.path.getmtime(f) != mtime:
            ...
            # Added this sleep to make sure all the files have been written to.
            # In the past the script would restart while the files were being
            # overwritten it would crash
            time.sleep(2)
